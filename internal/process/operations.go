...
func (s *Service) CognitiveRoutingDecided(
	...
	agentID id.AgentID,
	expected *uint64,
	payload CognitiveRoutingDecidedPayload,
	meta CommandMeta,
) (State, error) {
	return s.transition(ctx, "process.cognitive_routing_decided", agentID, expected, EventCognitiveRoutingDecided, payload, meta)
}
