...
func (o Objective) normalized() Objective {
	if o == (Objective{}) {
		return Objective{CorrectnessWeight: 0.65, QualityWeight: 0.35, MinCorrectness: 1}
	}
	return o
}
...
func (o Objective) Validate() error {
	o = o.normalized()
	if o.MinCorrectness < 0 || o.MinCorrectness > 1 {
		return errors.New("min correctness must be within [0,1]")
	}
	for _, w := range []float64{o.CorrectnessWeight, o.QualityWeight, o.LatencyWeight, o.MoneyWeight, o.TokenWeight} {
		if w < 0 {
			return errors.New("objective weights must be non-negative")
		}
	}
	return nil
}
...
func Evaluate(metrics Metrics, objective Objective, at time.Time) Evaluation {
	...
	// Benefits are positive, resource costs are negative. Scale integer costs so
	// ordinary v0 values remain numerically comparable without hidden tuning.
	...
	reason := ""
	if !eligible {
		reason = fmt.Sprintf("correctness %.4f below floor %.4f", metrics.Correctness, o.MinCorrectness)
	}
...
func SelectWinner(branches []Branch) (id.ForkID, error) {
	...
	for _, b := range branches {
		if b.Evaluation != nil && b.Evaluation.Eligible {
	...
	sort.Slice(eligible, func(i, j int) bool {
		if eligible[i].Evaluation.Score == eligible[j].Evaluation.Score {
			return strings.Compare(eligible[i].ID.String(), eligible[j].ID.String()) < 0
		}
