...
func (m *Manager) CreateCheckpoint(ctx context.Context, req CheckpointRequest) (Checkpoint, error) {
	...
	for _, ref := range processFrontier.CompletedResultRefs {
		completed[ref] = struct{}{}
	}
	for _, required := range req.RequiredResults {
		if required.Ref == "" {
			return Checkpoint{}, errors.New("required result reference is empty")
		}
	...
	frontier := ExecutionFrontier{
		...
		ProcessVersion:        state.Version,
		LedgerSequence:        processFrontier.LedgerSequence,
	...
	cp := Checkpoint{
		...
		ProcessStateJSON: append(json.RawMessage(nil), body...),
...
func (m *Manager) createGroupLocked(ctx context.Context, checkpointID id.CheckpointID) (Group, error) {
	...
	if cp.Class != CheckpointForkable && cp.Class != CheckpointCommittable {
		return Group{}, ErrUnsafeCheckpoint
	}
...
func (m *Manager) forkWorkspaceLocked(ctx context.Context, req ForkRequest) (Branch, world.TransactionalWorld, error) {
	if m.runtime == nil || m.budgets == nil || m.authority == nil {
		return Branch{}, nil, errors.New("fork runtime, budget store and authority provider are required")
	}
	if !req.BudgetLimit.Valid() {
		return Branch{}, nil, errors.New("fork budget must be non-negative")
	}
	...
	if g.State != GroupOpen {
		return Branch{}, nil, fmt.Errorf("%w: group is %s", ErrInvalidState, g.State)
	}
	...
	now := m.now().UTC()
	branchIntent := intersectIntent(cp.Authority.Intent, currentIntent)
	...
	defer func() {
		if reserved {
			_ = m.budgets.Release(reservation)
		}
	}()
	...
	defer func() {
		if worldOwned {
			_ = world.ReleaseWorkspaceBranch(ctx, rawWorld.BranchRef())
		}
	}()

	var source agentprocess.State
	if err := json.Unmarshal(cp.ProcessStateJSON, &source); err != nil {
		return Branch{}, nil, fmt.Errorf("decode checkpoint process state: %w", err)
	}
	...
	secure := world.NewSecureTransactionalWorld(world.NewAuthorizer(branchIntent, branchGrants, nil), rawWorld, m.now)
	...
	b := Branch{
		...
		WorldID:           worldID,
		WorldRef:          speculative.BranchRef(),
		Authority:         AuthoritySnapshot{Intent: branchIntent, Capabilities: append([]world.Capability(nil), branchGrants...), CapturedAt: now},
...
func (m *Manager) Spend(ctx context.Context, forkID id.ForkID, usage scheduler.Resources) (Branch, error) {
	...
	defer m.mu.Unlock()
	if !usage.Valid() {
		return Branch{}, errors.New("fork usage must be non-negative")
	}
	b, err := m.store.Branch(ctx, forkID)
	if err != nil {
		return Branch{}, err
	}
	if b.State != BranchOpen || b.BudgetSettled {
		return Branch{}, fmt.Errorf("%w: budget spend requires open unsettled branch", ErrInvalidState)
	}
	next := b.BudgetSpent.Add(usage)
	if !next.Fits(b.BudgetLimit) {
		return Branch{}, ErrBudgetExhausted
	}
...
func (m *Manager) PutOverlay(ctx context.Context, forkID id.ForkID, entry OverlayEntry) (Branch, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	if entry.Kind == "" || entry.Key == "" || len(entry.Value) == 0 {
		return Branch{}, errors.New("overlay kind, key and value are required")
	}
	if entry.Promotable && entry.Kind == OverlayMemory && entry.ProvenanceRef == "" {
		return Branch{}, errors.New("promotable memory requires provenance")
	}
	b, err := m.store.Branch(ctx, forkID)
	if err != nil {
		return Branch{}, err
	...
	if b.State != BranchOpen {
		return Branch{}, fmt.Errorf("%w: overlay write requires OPEN branch", ErrInvalidState)
	}
...
func (m *Manager) Evaluate(ctx context.Context, forkID id.ForkID, metrics Metrics, objective Objective, evidenceRef string) (Branch, error) {
	...
	if evidenceRef == "" {
		return Branch{}, errors.New("objective evaluation evidence reference is required")
	}
	if metrics.Correctness < 0 || metrics.Correctness > 1 || metrics.Quality < 0 || metrics.Quality > 1 || metrics.LatencyMS < 0 || metrics.MoneyMicros < 0 || metrics.Tokens < 0 {
		return Branch{}, errors.New("invalid evaluation metrics")
	}
	b, err := m.store.Branch(ctx, forkID)
	if err != nil {
		return Branch{}, err
	...
	if b.State != BranchOpen {
		return Branch{}, fmt.Errorf("%w: evaluation requires OPEN branch", ErrInvalidState)
	}
	e := Evaluate(metrics, objective, m.now())
...
func (m *Manager) Select(ctx context.Context, groupID id.ForkGroupID) (Branch, error) {
	...
	if g.State != GroupOpen {
		return Branch{}, fmt.Errorf("%w: group cannot select from %s", ErrInvalidState, g.State)
	}
	...
	if len(branches) == 0 {
		return Branch{}, ErrNoEligibleWinner
	}
	for _, b := range branches {
		if b.State != BranchEvaluated || b.Evaluation == nil {
			return Branch{}, fmt.Errorf("%w: every fork must be evaluated before selection", ErrInvalidState)
		}
	}
	...
	for _, b := range branches {
		if b.ID == winnerID {
			b.State = BranchSelected
			winner = b
		} else {
			b.State = BranchDiscarded
		}
...
func (m *Manager) BeginPromotion(ctx context.Context, forkID id.ForkID, branchWorld world.TransactionalWorld) (agenttx.Transaction, error) {
	...
	if m.transactions == nil {
		return agenttx.Transaction{}, errors.New("transaction manager is required")
	}
	b, err := m.store.Branch(ctx, forkID)
	if err != nil {
		return agenttx.Transaction{}, err
	}
	if b.State != BranchSelected || b.Evaluation == nil || !b.Evaluation.Eligible {
		return agenttx.Transaction{}, ErrPromotionRequired
	}
	g, err := m.store.Group(ctx, b.GroupID)
	if err != nil {
		return agenttx.Transaction{}, err
	}
	if g.WinnerForkID != b.ID || g.State != GroupEvaluated {
		return agenttx.Transaction{}, ErrPromotionRequired
	}
	cp, err := m.store.Checkpoint(ctx, g.CheckpointID)
	if err != nil {
	...
	if branchWorld == nil || branchWorld.BranchRef().WorldID != b.WorldID || branchWorld.BranchRef().BaseIdentity != cp.Frontier.WorldSnapshotRef.BaseIdentity {
		return agenttx.Transaction{}, errors.New("promotion world does not match selected fork/base")
	}
	...
	if err != nil {
		g.State = GroupEvaluated
		_ = m.store.UpdateGroup(ctx, g)
		return agenttx.Transaction{}, err
	}
	return tx, nil
}
...
func (m *Manager) FinalizePromotion(ctx context.Context, forkID id.ForkID, committed agenttx.Transaction) error {
	...
	if err := m.settleBudgetLocked(ctx, b); err != nil {
		return err
	}
	b, err = m.store.Branch(ctx, forkID)
	if err != nil {
...
func (m *Manager) PromoteCognitive(ctx context.Context, forkID id.ForkID, promoter CognitivePromoter) error {
	...
	defer m.mu.Unlock()
	if promoter == nil {
		return errors.New("cognitive promoter is required")
	}
	b, err := m.store.Branch(ctx, forkID)
	if err != nil {
		return err
	}
	if b.State != BranchPromoted {
		return ErrPromotionRequired
	}
	g, err := m.store.Group(ctx, b.GroupID)
	if err != nil {
	...
	for i := range b.CognitiveOverlay {
		...
		if !entry.Promotable || entry.Promoted {
			continue
		}
		opKey := "fork-promote:" + b.ID.String() + ":" + entry.Key
		if err := promoter.PromoteForkArtifact(ctx, opKey, cp, b, entry); err != nil {
			return err
		}
...
func (m *Manager) PromoteableOverlay(ctx context.Context, forkID id.ForkID) ([]OverlayEntry, error) {
	...
	if b.State != BranchSelected && b.State != BranchPromoted {
		return nil, ErrPromotionRequired
	}
	out := make([]OverlayEntry, 0)
	for _, entry := range b.CognitiveOverlay {
		if entry.Promotable {
...
func (m *Manager) CleanupGroup(ctx context.Context, groupID id.ForkGroupID) error {
	...
	if g.State == GroupPromoting {
		return fmt.Errorf("%w: cannot cleanup during promotion", ErrInvalidState)
	}
	branches, err := m.store.Branches(ctx, groupID)
	if err != nil {
		return err
	...
	for _, b := range branches {
		if err := cleanupBranchWorld(ctx, b); err != nil {
			return err
		}
		if err := m.settleBudgetLocked(ctx, b); err != nil {
			return err
	...
	if g.State != GroupPromoted {
		g.State = GroupDiscarded
	}
...
func (m *Manager) PurgeGroup(ctx context.Context, groupID id.ForkGroupID, now time.Time) error {
	...
	if g.State != GroupPromoted && g.State != GroupDiscarded {
		return fmt.Errorf("%w: group is not terminal", ErrInvalidState)
	}
	cp, err := m.store.Checkpoint(ctx, g.CheckpointID)
	if err != nil {
		return err
	}
	if cp.RetainUntil != nil && now.UTC().Before(cp.RetainUntil.UTC()) {
		return fmt.Errorf("retention window has not expired")
	}
	branches, err := m.store.Branches(ctx, groupID)
	if err != nil {
		return err
	...
	for _, b := range branches {
		if !b.BudgetSettled {
			return fmt.Errorf("%w: fork %s budget is not settled", ErrInvalidState, b.ID)
		}
		if b.WorldRef.Type == world.TypeWorkspace {
			if _, err := os.Stat(b.WorldRef.Ref); err == nil {
				return fmt.Errorf("%w: fork %s world still exists", ErrInvalidState, b.ID)
			} else if !errors.Is(err, os.ErrNotExist) {
				return err
			}
		}
		if err := m.store.DeleteBranch(ctx, b.ID); err != nil {
			return err
		}
	}
...
func (m *Manager) PurgeCheckpoint(ctx context.Context, checkpointID id.CheckpointID, now time.Time) error {
	...
	if err := world.ReleaseWorkspaceSnapshot(ctx, cp.Frontier.WorldSnapshotRef); err != nil {
		return err
	}
	return m.store.DeleteCheckpoint(ctx, checkpointID)
}

func (m *Manager) settleBudgetLocked(ctx context.Context, b Branch) error {
	if b.BudgetSettled {
		return nil
	}
	if m.budgets == nil || b.BudgetReservation == "" {
		return errors.New("fork budget reservation is missing")
	}
	err := m.budgets.Settle(b.BudgetReservation, b.BudgetSpent)
	if err != nil && !errors.Is(err, scheduler.ErrReservationSettled) {
		return err
	}
	b.BudgetSettled, b.UpdatedAt = true, m.now().UTC()
	return m.store.UpdateBranch(ctx, b)
}
...
func cleanupBranchWorld(ctx context.Context, b Branch) error {
	...
	return world.ReleaseWorkspaceBranch(ctx, b.WorldRef)
}
...
func intersectCapabilities(historical, current []world.Capability, now time.Time) []world.Capability {
	...
	for _, grant := range current {
		...
		if world.ValidateDelegation(historical, []world.Capability{grant}, now) {
...
func intersectIntent(historical, current world.Intent) world.Intent {
	...
	return world.Intent{
		...
		Goal:               goal,
		AcceptanceCriteria: unionStrings(historical.AcceptanceCriteria, current.AcceptanceCriteria),
		AllowedDomains:     intersectOptionalAllowlist(historical.AllowedDomains, current.AllowedDomains),
		ForbiddenDomains:   unionStrings(historical.ForbiddenDomains, current.ForbiddenDomains),
	}
}

func intersectOptionalAllowlist(historical, current []string) []string {
	switch {
	case len(historical) == 0 && len(current) == 0:
		return nil
	case len(historical) == 0:
		return sortedUnique(current)
	case len(current) == 0:
		return sortedUnique(historical)
	}
	allowed := make(map[string]struct{}, len(historical))
