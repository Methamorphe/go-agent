...
func NewManager(store Store, processes agentprocess.Store, transactions *agenttx.Manager, ids IDGenerator, now func() time.Time) *Manager {
	if now == nil { now = time.Now }
	return &Manager{store: store, processes: processes, transactions: transactions, ids: ids, now: now}
}
...
func (m *Manager) CreateCheckpoint(ctx context.Context, sourceAgentID id.AgentID, snapshot world.BranchRef, contextFrontier []id.ContextPageID, retainUntil *time.Time) (Checkpoint, error) {
	...
	if state.Lease != nil || state.Wait != nil || state.Sleep != nil {
		return Checkpoint{}, fmt.Errorf("%w: process has active execution/wait state", ErrInvalidState)
	}
	...
	cp := Checkpoint{
		ID: checkpointID, SourceAgentID: sourceAgentID, RootAgentID: state.RootAgentID,
		ProcessVersion: state.Version, LedgerSequence: sequence, LastEventID: state.LastEventID,
		WorldRef: snapshot, ContextFrontier: append([]id.ContextPageID(nil), contextFrontier...),
		StateHash: hex.EncodeToString(sum[:]), CreatedAt: m.now().UTC(), RetainUntil: retainUntil,
	}
...
func (m *Manager) ForkWorkspace(ctx context.Context, req ForkRequest) (Branch, *world.WorkspaceWorld, error) {
	...
	agentID, err := m.ids.Agent(); if err != nil { return Branch{}, nil, err }
	...
	now := m.now().UTC()
	b := Branch{ID: forkID, GroupID: g.ID, AgentID: agentID, WorldID: worldID, WorldRef: branchWorld.BranchRef(), BudgetLimit: req.BudgetLimit, State: BranchOpen, CreatedAt: now, UpdatedAt: now}
...
func (m *Manager) Select(ctx context.Context, groupID id.ForkGroupID) (Branch, error) {
	...
	for _, b := range branches {
		switch {
		case b.ID == winnerID:
			b.State = BranchSelected; winner = b
		case b.State == BranchEvaluated:
...
// PromoteableOverlay returns only explicit branch-local artifacts marked for
// promotion. Caller must persist them through the owning cognitive subsystem.
...
// RestoreAsNewTimeline returns a fresh isolated agent/world derived from the
// checkpoint. It never truncates or mutates the source process ledger.
func (m *Manager) RestoreAsNewTimeline(ctx context.Context, checkpointID id.CheckpointID, budget scheduler.Resources, workRoot string) (Group, Branch, *world.WorkspaceWorld, error) {
	...
	b, w, err := m.ForkWorkspace(ctx, ForkRequest{GroupID: g.ID, BudgetLimit: budget, WorkRoot: workRoot})
	return g, b, w, err
}
...
func (m *Manager) CleanupGroup(ctx context.Context, groupID id.ForkGroupID) error {
	...
	for _, b := range branches {
		if b.WorldRef.Type == world.TypeWorkspace {
			if w, reopenErr := world.ReopenWorkspaceWorld(b.WorldRef); reopenErr == nil { _ = w.Rollback(ctx) }
		}
		if err := m.store.DeleteBranch(ctx, b.ID); err != nil { return err }
	}
	g.State, g.UpdatedAt = GroupDiscarded, m.now().UTC()
	if err := m.store.UpdateGroup(ctx, g); err != nil { return err }
