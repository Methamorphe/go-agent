...
// ReleaseWorkspaceSnapshot removes only the retained checkpoint worktree. Git
// objects remain reachable through child fork refs while those forks exist.
func ReleaseWorkspaceSnapshot(ctx context.Context, ref BranchRef) error {
	...
	if meta.Repository == "" || meta.Worktree == "" { return nil }
	return removeWorkspaceWorktree(ctx, meta.Repository, meta.Worktree)
}
