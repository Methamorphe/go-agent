...
func (s *Store) PutCheckpoint(ctx context.Context, cp cogfork.Checkpoint) error {
	...
	authorityJSON, err := json.Marshal(cp.Authority)
	if err != nil { return err }
	var retain any
...
checkpoint_id, class, source_agent_id, root_agent_id, frontier_json, authority_json,
last_event_id, process_state_json, state_hash, created_at, retain_until)
VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, cp.ID.String(), string(cp.Class), cp.SourceAgentID.String(), cp.RootAgentID.String(), frontierJSON, authorityJSON, cp.LastEventID.String(), []byte(cp.ProcessStateJSON), cp.StateHash, formatForkTime(cp.CreatedAt), retain)
...
func (s *Store) Checkpoint(ctx context.Context, checkpointID id.CheckpointID) (cogfork.Checkpoint, error) {
	...
	var checkpoint, class, source, root, lastEvent, created string
	var frontierJSON, authorityJSON, processJSON []byte
	var retain sql.NullString
	err := s.db.QueryRowContext(ctx, `SELECT checkpoint_id, class, source_agent_id, root_agent_id, frontier_json, authority_json, last_event_id, process_state_json, state_hash, created_at, retain_until
FROM cognitive_fork_checkpoints WHERE checkpoint_id = ?`, checkpointID.String()).Scan(&checkpoint, &class, &source, &root, &frontierJSON, &authorityJSON, &lastEvent, &processJSON, &cp.StateHash, &created, &retain)
	...
	cp.ID, cp.Class, cp.SourceAgentID, cp.RootAgentID, cp.LastEventID = id.CheckpointID(checkpoint), cogfork.CheckpointClass(class), id.AgentID(source), id.AgentID(root), id.EventID(lastEvent)
	if err := json.Unmarshal(frontierJSON, &cp.Frontier); err != nil { return cogfork.Checkpoint{}, err }
	if err := json.Unmarshal(authorityJSON, &cp.Authority); err != nil { return cogfork.Checkpoint{}, err }
...
func (s *Store) CreateGroup(ctx context.Context, g cogfork.Group) error {
	_, err := s.db.ExecContext(ctx, `INSERT INTO cognitive_fork_groups(group_id, checkpoint_id, source_agent_id, state, winner_fork_id, selection_reason, created_at, updated_at) VALUES(?, ?, ?, ?, ?, ?, ?, ?)`, g.ID.String(), g.CheckpointID.String(), g.SourceAgentID.String(), string(g.State), nullableFork(g.WinnerForkID.String()), g.SelectionReason, formatForkTime(g.CreatedAt), formatForkTime(g.UpdatedAt))
...
func (s *Store) Group(ctx context.Context, groupID id.ForkGroupID) (cogfork.Group, error) {
	...
	err := s.db.QueryRowContext(ctx, `SELECT group_id, checkpoint_id, source_agent_id, state, winner_fork_id, selection_reason, created_at, updated_at FROM cognitive_fork_groups WHERE group_id = ?`, groupID.String()).Scan(&group, &checkpoint, &source, &state, &winner, &g.SelectionReason, &created, &updated)
...
func (s *Store) UpdateGroup(ctx context.Context, g cogfork.Group) error {
	res, err := s.db.ExecContext(ctx, `UPDATE cognitive_fork_groups SET state = ?, winner_fork_id = ?, selection_reason = ?, updated_at = ? WHERE group_id = ?`, string(g.State), nullableFork(g.WinnerForkID.String()), g.SelectionReason, formatForkTime(g.UpdatedAt), g.ID.String())
	if err != nil { return err }
...
func (s *Store) CreateBranch(ctx context.Context, b cogfork.Branch) error {
	...
	if err != nil { return err }
	authorityJSON, err := json.Marshal(b.Authority)
	if err != nil { return err }
...
fork_id, group_id, agent_id, world_id, world_ref_json, authority_json,
budget_limit_money_micros, budget_limit_tokens, budget_spent_money_micros, budget_spent_tokens,
budget_reservation_id, budget_settled, state, evaluation_json, cognitive_overlay_json, created_at, updated_at)
VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, b.ID.String(), b.GroupID.String(), b.AgentID.String(), b.WorldID.String(), worldJSON, authorityJSON, b.BudgetLimit.MoneyMicros, b.BudgetLimit.Tokens, b.BudgetSpent.MoneyMicros, b.BudgetSpent.Tokens, string(b.BudgetReservation), forkBoolInt(b.BudgetSettled), string(b.State), evalJSON, overlayJSON, formatForkTime(b.CreatedAt), formatForkTime(b.UpdatedAt))
...
func (s *Store) UpdateBranch(ctx context.Context, b cogfork.Branch) error {
	...
	authorityJSON, err := json.Marshal(b.Authority)
	if err != nil { return err }
	res, err := s.db.ExecContext(ctx, `UPDATE cognitive_fork_branches SET authority_json = ?, budget_spent_money_micros = ?, budget_spent_tokens = ?, budget_settled = ?, state = ?, evaluation_json = ?, cognitive_overlay_json = ?, updated_at = ? WHERE fork_id = ?`, authorityJSON, b.BudgetSpent.MoneyMicros, b.BudgetSpent.Tokens, forkBoolInt(b.BudgetSettled), string(b.State), evalJSON, overlayJSON, formatForkTime(b.UpdatedAt), b.ID.String())
	if err != nil { return err }
	n, err := res.RowsAffected()
	if err != nil { return err }
	if n != 1 { return cogfork.ErrNotFound }
...
const forkBranchSelect = `SELECT fork_id, group_id, agent_id, world_id, world_ref_json, authority_json,
budget_limit_money_micros, budget_limit_tokens, budget_spent_money_micros, budget_spent_tokens,
budget_reservation_id, budget_settled, state, evaluation_json, cognitive_overlay_json, created_at, updated_at FROM cognitive_fork_branches`
...
func scanForkBranch(row forkRowScanner) (cogfork.Branch, error) {
	...
	var forkID, groupID, agentID, worldID, reservation, state, created, updated string
	var worldJSON, authorityJSON, evalJSON, overlayJSON []byte
	var budgetSettled int
	if err := row.Scan(&forkID, &groupID, &agentID, &worldID, &worldJSON, &authorityJSON, &b.BudgetLimit.MoneyMicros, &b.BudgetLimit.Tokens, &b.BudgetSpent.MoneyMicros, &b.BudgetSpent.Tokens, &reservation, &budgetSettled, &state, &evalJSON, &overlayJSON, &created, &updated); err != nil { return cogfork.Branch{}, err }
	...
	b.BudgetReservation, b.BudgetSettled = scheduler.ReservationID(reservation), budgetSettled != 0
	if err := json.Unmarshal(worldJSON, &b.WorldRef); err != nil { return cogfork.Branch{}, err }
	if err := json.Unmarshal(authorityJSON, &b.Authority); err != nil { return cogfork.Branch{}, err }
	if len(evalJSON) > 0 {
