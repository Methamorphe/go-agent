...
func ref(t *testing.T, value string) CognitiveRef {
	t.Helper(); r, err := ParseCognitiveRef(value); if err != nil { t.Fatal(err) }; return r
}
...
func TestCF001StableCognitiveReferences(t *testing.T) {
	for _, value := range []string{"ctx://ctx_1", "belief://blf_1", "evidence://evd_1", "object://sha256:abc", "event://evt_1", "checkpoint://chk_1", "agent://agt_1"} {
		r, err := ParseCognitiveRef(value); if err != nil || r.String() != value { t.Fatalf("parse %q: %v %q", value, err, r) }
	}
	if _, err := ParseCognitiveRef("file:///etc/passwd"); !errs.IsCode(err, errs.CodeInvalidArgument) { t.Fatalf("expected invalid scheme, got %v", err) }
}
...
func TestCF002ReferenceFaultLeasesPageForNextBuild(t *testing.T) {
	...
	if result.State != FaultResolved || len(result.PageIDs) != 1 || result.PageIDs[0] != page.ID { t.Fatalf("unexpected resolution: %+v", result) }
	...
	if err != nil { t.Fatal(err) }
	found := false
	for _, selected := range set.Pages { if selected.Meta.ID == page.ID && selected.Reason == "context_lease" { found = true } }
	if !found { t.Fatal("faulted page was not leased into next working set") }
}
...
func TestCF003EvidenceFaultPagesRawCompactionSources(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if len(result.PageIDs) != 2 || result.PageIDs[0] != a.ID || result.PageIDs[1] != b.ID { t.Fatalf("raw evidence not paged: %+v", result.PageIDs) }
}
...
func TestCF004FreshnessFaultFollowsSupersession(t *testing.T) {
	...
	repo.mu.Lock(); meta := repo.pages[old.ID]; next := fresh.ID; meta.SupersededBy = &next; repo.pages[old.ID] = meta; repo.mu.Unlock()
	...
	if err != nil { t.Fatal(err) }
	if len(result.PageIDs) != 1 || result.PageIDs[0] != fresh.ID { t.Fatalf("freshness did not follow supersession: %+v", result) }
}

type staticResolver struct { page id.ContextPageID; dependency CognitiveRef }
...
func TestCF005DependencyPagingIsBoundedAndResolverIndependent(t *testing.T) {
	...
	if err := manager.RegisterResolver(RefBelief, staticResolver{page: beliefPage.ID, dependency: ref(t, "ctx://"+evidencePage.ID.String())}); err != nil { t.Fatal(err) }
	...
	if err != nil { t.Fatal(err) }
	if len(result.PageIDs) != 2 { t.Fatalf("expected resolver page + dependency, got %+v", result.PageIDs) }
}
...
func TestCF006FaultStormStopsRepeatedPaging(t *testing.T) {
	...
	for i := 0; i < DefaultFaultBudget().MaxRepeatedSetHits; i++ { if _, err := manager.ResolveFault(context.Background(), request); err != nil { t.Fatalf("fault %d failed early: %v", i, err) } }
	result, err := manager.ResolveFault(context.Background(), request)
	if err == nil || !errors.Is(err, err) || !errs.IsCode(err, errs.CodeResourceExhausted) { t.Fatalf("expected storm rejection, got result=%+v err=%v", result, err) }
	if result.State != FaultBudgetExceeded { t.Fatalf("expected budget exceeded state, got %s", result.State) }
}
...
func TestCF007RepresentationFaultRequiresProjectionWhenTooLarge(t *testing.T) {
	...
	if err == nil || result.State != FaultNeedsProjection { t.Fatalf("expected projection state, got %+v err=%v", result, err) }
}
